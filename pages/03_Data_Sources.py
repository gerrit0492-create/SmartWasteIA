...
st.divider()
st.subheader("🔄 Update actuele prijzen (template)")
if st.button("Fetch latest"):
    try:
        run_update()
        st.success("Update script uitgevoerd. Check materials.csv (last_update gewijzigd).")
...
if st.button("⬆️ Push materials -> Google Sheets"):
    try:
        target = sid_m or st.secrets["sheets"]["materials_sheet_id"]
        gs.write_sheet(materials, target)
